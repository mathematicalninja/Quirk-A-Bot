...
function gotMessage(msg) {
	...
	if (msg.channel.id == process.env.DICE_ROLLING_CHANNEL) { //Dice Rolling channel

		// Add in a roll 3d10 style roller (with multi support "roll 3d4, 2d6")

		if (msg.content.match(DICE_ROLL_REGEX)) { //Hungry Dice and Clean
			let playerDice = regexToDice(msg.content.match(DICE_ROLL_REGEX));
			msg.reply(doTheRolling(playerDice));

		} else if (msg.content.match(DICE_SINGLE_ROLL_REGEX)) { //Just Clean Dice
...
function regexToDice(RegexMatch) { // turns the regex's text into js integers
...
function showRolls(Results) {
	...
	if (!((Results[1][0]) == undefined)) {
		...
		if (cleanString != "") { //if there are clean dice, newline before the Hungry dice
			hungryString = "\nHungry Dice: ";
		} else {
		...
		for (let i in Results[1]) {
			if (i != Len - 1) { // this isn't the final of the hungry dice
